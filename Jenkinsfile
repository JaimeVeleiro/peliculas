        ...
        stage('Build') {
            ...
            steps {
                sh '''
                    ...
                    ssh -p $SSH_PORT -i ${SSH_PRIVATE_KEY} ${SSH_USER}@${SSH_HOST} "cd ${VH_PATH} && docker image rm veleiroruiz_php && ./veleiroruiz-parada.sh 3 && docker build -t veleiroruiz_php . && ./veleiroruiz-arranque.sh 3"
